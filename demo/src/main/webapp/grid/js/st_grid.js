...
grid.prototype = {
        ...
        if (data && typeof data == 'object') {
            ...
            if (rows && rows.length > 0) {
                ...
                for ( var i = 0, len = rows.length; i < len; i++) {
                	this.autoIndex =this.params.lines*(this.params[this.mapKeys.currentPage]-1)+i+1;
                	
                    var model = rows[i];
                    model["autoIndex"] = this.autoIndex;
                ...
                $(this.holderNode.tBodies[0]).html(
                        ...
                        '<tr><td style="text-align:center;" colspan="' + colspan + '">无匹配的内容</td></tr>');
    ...
    parseTemplate : function(model) {
        ...
        return this.template.replace(this.trReg, function() {
            ...
            if (fun) {
            	...
            	if(prop =="autoIndex"){
            		model[prop] = that.autoIndex;
            		return that.autoIndex;
            	}else{
    ...
    addPageEvent : function(btns) {
        ...
        if(btns[5]){
        	btns[5].id = "page-row-select";
        	$(btns[5]).html( "<option value='20'>20</option><option value='50'>50</option><option value='100'>100</option>");
        	btns[5].onchange=  function() {
      		  that.send({
      	            lines : this.value,
      	          currentPage : 1
      	        });
            };
        	btns[5].value = that.params.lines;
        }
    },
    ...
    send : function(params) {
        ...
        if(orderBy){
            ...
            for ( var i = 0, len = ths.length; i < len; i++) {
                ...
                if (column) {
                    ...
                    if (column == orderBy) {
                             ...
                             currentSpan = span;
                             this.setOrderColumn(span, 'sort-' + this.params[this.mapKeys.order]);
                    } else {
            ...
            // 移至 566 行 可能多个图标同 一个 排序作用
        //    currentSpan && this.setOrderColumn(currentSpan, 'sort-' + this.params[this.mapKeys.order]);

        }
        if($("#page-row-select")[0]){
            this.params.lines= $("#page-row-select").val();
        } else {
            // 默认 lines
            this.params.lines = 20;
        }
        $.ajax({
    ...
    goTo : function(page) {
        if (!/^\d+$/.test(page)){
        	page = this.params[this.mapKeys.currentPage];
        }
        if (page < 1)
