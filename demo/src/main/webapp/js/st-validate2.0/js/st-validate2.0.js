;var validator = (function(){
	...
	eleWapper.prototype = {
				...
				$.ajax({
					...
					success : function(result){
						if(typeof result == 'boolean'){
							flg = this.updateMsg(result ? 'valid' : 'error');
						}else{
							flg = this.updateMsg('remote');
						}
					}
				});
				return flg;
		...
		getVal : function(){
			...
			if(type == 'text' || type == 'textarea' || type == 'hidden' || (type && type.indexOf('select') == 0)){
