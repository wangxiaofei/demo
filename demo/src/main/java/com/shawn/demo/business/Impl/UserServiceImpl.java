...
import com.shawn.demo.utils.MD5Utils;
import com.shawn.demo.utils.MailUtils;
...
//            }
    ...
    /**
     * 重置密码token 校验
     * 场景1：token 不为空并且等于 MD5(userId+oldPassword) true
     * 其它场景：返回false
     * @param userId
     * @param token
     * @return
    ...
    private boolean resetPwdTokenChecker(Integer userId, String token) {
        ...
        if (StringUtils.isBlank(token) || null == user
                || !token.equals(this.getRestPwdToken(userId, user.getPassword()))) {
            return false;
        }
        return true;
    }
